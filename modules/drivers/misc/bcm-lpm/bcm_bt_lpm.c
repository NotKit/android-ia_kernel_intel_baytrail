...
#define INVALID_GPIO ((unsigned int)-1)
...
static void activate_irq_handler(void)
{
	...
	if (ret < 0) {
		...
		bt_lpm.gpio_wake = INVALID_GPIO;
		gpio_free(bt_lpm.gpio_host_wake);
		bt_lpm.gpio_host_wake = INVALID_GPIO;
	} else {
		bt_lpm.int_host_wake_requested = true;
	}
}
...
{
	if (bt_lpm.int_host_wake_requested) {
		free_irq(bt_lpm.int_host_wake, NULL);
		bt_lpm.int_host_wake_requested = false;
	}

	if (hrtimer_active(&bt_lpm.enter_lpm_timer))
