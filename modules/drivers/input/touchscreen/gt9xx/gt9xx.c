...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
    ...
    {
        wake_lock_timeout(&ts->wake_lock, msecs_to_jiffies(500));
    }
    queue_work(goodix_wq, &ts->work);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    usleep_range(ns, ns + 1);           // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    GTP_GPIO_OUTPUT(GTP_INT_PORT, client->addr == 0x14);

    usleep_range(100, 101);             // T3: > 100us
    GTP_GPIO_OUTPUT(GTP_RST_PORT, 1);

    usleep_range(5000, 5001);           // T4: > 5ms
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
    ...
    if (doze_status == DOZE_ENABLED) {
        GTP_DEBUG("Doze already enabled!");
        return 1;
    }
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    ret  = request_irq(ts->client->irq,
                       ...
                       irq_table[ts->int_trigger_type] | IRQF_NO_SUSPEND,
                       ts->client->name,
                       ts);
    if (ret)
    {
...
#if GTP_GESTURE_WAKEUP
static ssize_t attr_gest_wakeup_en_store(struct device *dev,
                               ...
                               const char *buf, size_t size)
{
    ...
    if (ts->gtp_is_suspend && atomic_read(&ts->gest_wakeup_is_enable) != val)
    {
        ret = gtp_wakeup_sleep(ts);
        if (ret < 0)
        {
            GTP_ERROR("GTP wake up failed.");
        }
    ...
    atomic_set(&ts->gest_wakeup_is_enable, val);
...
static ssize_t attr_gest_wakeup_en_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    struct goodix_ts_data *ts = dev_get_drvdata(dev);
    return sprintf(buf, "%d\n", atomic_read(&ts->gest_wakeup_is_enable));
}

static struct device_attribute attributes[] = {
        __ATTR(gesture_wakeup_enable, S_IRUGO | S_IWUSR, attr_gest_wakeup_en_show,
    							attr_gest_wakeup_en_store),
};
#endif
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if GTP_GESTURE_WAKEUP
    // set gesture wake up as enabled by default
    atomic_set(&ts->gest_wakeup_is_enable, 1);

    ret = device_create_file(&client->dev, attributes);
    if (ret < 0)
    {
        GTP_ERROR("sysfs device registration failed.");
    }
#endif
    ...
    wake_lock_init(&ts->wake_lock, WAKE_LOCK_SUSPEND, "tp_wake_lock");
...
#if GTP_GESTURE_WAKEUP
    device_remove_file(&client->dev, attributes);
#endif
...
#if GTP_GESTURE_WAKEUP
    if (atomic_read(&ts->gest_wakeup_is_enable))
    {
        ret = gtp_enter_doze(ts);
    }
    else
