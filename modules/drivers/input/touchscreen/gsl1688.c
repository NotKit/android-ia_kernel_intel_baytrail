...
static u16 TS_RST_GPIO = 128;

static u16 TS_INT_GPIO = 133;
...
/*	struct i2c_client *client = ddata->client;
	...
	check_mem_data(client);
        enable_irq(client->irq);
*/  
    printk("[GSL] : gsl_ts_resume START\n");
    gsl_late_resume();
...
/*	int ret;
	...
	gpio_set_value(ddata->gpio_reset, 0);
*/  
    printk("[GSL] : gsl_ts_suspend START\n");
    gsl_early_suspend();
...
static int gsl_acpi_probe(struct gsl_ts_data *ts_data)
{
	...
	ts_data->gpio_reset = GPIO_TP_RESET;
	gpio_request(ts_data->gpio_reset, "gsl reset");

	TS_RST_GPIO = ts_data->gpio_reset;

	index++;

    ts_data->gpio_irq = GPIO_TP_IRQ;
	gpio_request(ts_data->gpio_irq, "gsl irq");
...
static int gsl_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{	
        ...
        //ddata->pdata = pdata;
	ddata->x_max = 715;//pdata->x_max - 1;//GSL_MAX_X;//pdata->x_max;
	ddata->y_max = 1115;//pdata->y_max - 1;//GSL_MAX_Y;//pdata->y_max;
