 ...
 *
 * $Id: dhd_sdio.c 443394 2013-12-16 15:59:25Z $
 */
...
static int concate_revision_bcm43362(dhd_bus_t *bus, char *fw_path, int fw_path_len, char *nv_path, int nv_path_len)
{
	uint32 chip_id, chip_ver;
#if defined(SUPPORT_MULTIPLE_CHIPS)
	char chipver_tag[10] = "_43362";
#else
	char chipver_tag[4] = {0, };
#endif /* defined(SUPPORT_MULTIPLE_CHIPS) */

	DHD_TRACE(("%s: BCM43362 Multiple Revision Check\n", __FUNCTION__));

	chip_id = bus->sih->chip;
	chip_ver = bus->sih->chiprev;

	if (chip_ver == 1) {
		DHD_ERROR(("----- CHIP bcm43362_A2 -----\n"));
	...
	char *fw_path, int fw_path_len, char *nv_path, int nv_path_len)
{
	...
	DHD_TRACE(("%s: BCM4354 Multiple Revision Check\n", __FUNCTION__));

	chip_id = bus->sih->chip;
	chip_ver = bus->sih->chiprev;

	if (chip_ver == 1) {
		DHD_ERROR(("----- CHIP bcm4354_A0 -----\n"));
...
concate_revision(dhd_bus_t *bus, char *fw_path, int fw_path_len, char *nv_path, int nv_path_len)
{
	...
	if (!bus || !bus->sih) {
        ...
        printk("Enter concate_reversion: bus->sih->chip=%d, chiprev=%d\n", bus->sih->chip, bus->sih->chiprev);
	if (!fw_path && !nv_path)
		return res;
	...
	case BCM43362_CHIP_ID:
		res = concate_revision_bcm43362(bus, fw_path, fw_path_len, nv_path, nv_path_len);
	default:
